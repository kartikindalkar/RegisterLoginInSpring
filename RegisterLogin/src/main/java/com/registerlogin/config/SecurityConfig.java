...
public class SecurityConfig {
    ...
    public SecurityFilterChain FilterChain(HttpSecurity http) throws Exception {
        ...
        /*
         * http.csrf(csrf -> { try { csrf.disable()
         * .authorizeHttpRequests().requestMatchers("/", "/register",
         * "/login","/saveUser")
         * .permitAll().requestMatchers("/user/**").authenticated(); } catch (Exception
         * e) { e.printStackTrace(); } }).formLogin(login -> login
       ...
       .requestMatchers("/**").permitAll().and().formLogin().loginPage("/login").loginProcessingUrl("/userLogin")
